document.addEventListener('DOMContentLoaded', () => {
    ...
    let isAdvancedMode = true; // Track current mode - changed to true by default
    ...
    function exportComparison(selectedItems) {
        try {
            ...
            const doc = new jsPDF('p','pt','a4'); const page={w:doc.internal.pageSize.getWidth(),h:doc.internal.pageSize.getHeight(),m:36}; let y=page.m;
            doc.setFillColor(59,130,246); doc.rect(0,0,page.w,80,'F'); doc.setTextColor(255,255,255); doc.setFont('helvetica','bold'); doc.setFontSize(20); doc.text('Project Comparison', page.m, 46); doc.setFontSize(10); doc.text(new Date().toLocaleString(), page.w-page.m-120, 62); y=96;
            const headers=['Property', ...selectedItems.map(i=>`${i.projectType.charAt(0).toUpperCase()+i.projectType.slice(1)} (${i.dimensions.length}×${i.dimensions.width})`)];
            const rows=[
                ['Dimensions (L×W×T)', ...selectedItems.map(i=>`${i.dimensions.length}×${i.dimensions.width}×${i.dimensions.thickness}`)],
                ['Volume (yd³)', ...selectedItems.map(i=>String(i.volume.cubicYards))],
                ['Cement (bags)', ...selectedItems.map(i=>String(i.materials.cementBags))],
                ['Sand (t)', ...selectedItems.map(i=>String(i.materials.sandTons))],
                ['Gravel (t)', ...selectedItems.map(i=>String(i.materials.gravelTons))],
                ['Labor ($)', ...selectedItems.map(i=>`$${Math.round(i.costs.labor)}`)],
                ['Total ($)', ...selectedItems.map(i=>`$${Math.round(i.costs.total)}`)]
            ];
            const tableW = page.w - page.m*2, colW = tableW / headers.length, rowH = 24;
            const drawRow = (cells, opts={}) => { if (y > page.h - 60) { doc.addPage(); y = page.m; }
                const bg = opts.header ? [59,130,246] : (opts.index % 2 ? [247,249,252] : [241,245,249]);
                doc.setFillColor(...bg); doc.setDrawColor(230, 235, 243); doc.roundedRect(page.m, y-16, tableW, rowH, 6, 6, 'FD');
                doc.setTextColor(...color);
                doc.text(text, page.m + padX, y + 1);
                y += h + 8;
                doc.setTextColor(30, 35, 40);
                doc.setFont('helvetica', 'normal');
            };
            drawRow(headers, { header:true }); rows.forEach((r,i)=>drawRow(r, { index:i }));
            doc.setFontSize(8); doc.setTextColor(140); 
            // remove footer text: "Generated by Professional Concrete Calculator"
            doc.save('comparison.pdf'); showToast('Comparison PDF exported!', 'success');
        } catch (error) { console.error('PDF export error:', error); showToast('Error exporting PDF. Please try again.', 'error'); }
    }
...
Total Cost: $${data.costs.total.toFixed(2)}`;
    ...
    function initializeEnhancedFeatures() {
        ...
        // Set advanced mode as default and ensure active tab shows correctly
        isAdvancedMode = true;
        ...
        if (advancedTab && normalTab && advancedContent && normalContent) {
            // Remove active from normal
            normalTab.classList.remove('active');
            normalContent.classList.remove('active');
            
            // Add active to advanced
